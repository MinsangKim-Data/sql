...
/* SELECT v.vendor_name, COUNT(vba.booth_number) AS booth_rentals
FROM vendor_booth_assignments vba
JOIN vendor v ON v.vendor_id = vba.vendor_id
GROUP BY v.vendor_name; 
-- this changes the vendor_id to vendor_name*/
...
--CREATE TABLE bumper_sticker_qualified AS
SELECT 
cp.customer_id,
...
--check what new_vendor looks like
SELECT * FROM new_vendor;

INSERT INTO new_vendor (vendor_id, vendor_name, vendor_type, vendor_owner_first_name, vendor_owner_last_name)
VALUES (10, 'Thomass Superfood Store', 'Fresh Focused', 'Thomas', 'Rosenthal')

--check what new_vendor looks like after addition of 10th row
SELECT * FROM new_vendor;
...
SELECT 
    customer_id,
    strftime('%m', market_date) AS month,   -- Extracts the month
    strftime('%Y', market_date) AS year     -- Extracts the year
FROM 
    customer_purchases;
